...
func (req CreateUserReq) Valid(ctx context.Context) validator.Evaluator {
	...
	eval.CheckField(validator.Matches(req.Email, validator.EmailRX), "email", "mus be a valid email")
	eval.CheckField(validator.NotBlank(req.Bio), "bio", "this field cannot be empty")
	eval.CheckField(
		validator.MinChars(req.Bio, 10) &&
		validator.MinChars(req.Bio, 255), 
		"bio", 
		"this field must have a length between 10 and 255",
	)
	eval.CheckField(validator.MinChars(req.Password, 8), "password", "password must be greater than 7 characters")
